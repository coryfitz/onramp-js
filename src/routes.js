// Framework-owned route generation used by generated OnRamp projects.
const fs = require('fs');
...
function buildImportPath(projectRoot, chosenRelFromApp) {
  ...
  // Drop the extension for clean route-module imports and resolver compatibility.
...
function generateRoutesConfig(projectRoot = process.cwd()) {
  ...
  const addRoute = (routePath, importPath) => {
    ...
    if (!routeComponents[componentKey]) {
      routeComponents[componentKey] = importPath;
    }
  ...
  // Metro splits dynamic imports into additional development bundles. Registering those
  // bundles with Fast Refresh can cause a native refresh loop, so native route modules are
  // included in the initial graph. Web keeps dynamic imports and its route-level code splitting.
  const eagerNativeRoutes = platform !== 'web';
  const componentEntries = Object.entries(routeComponents);
  const routeImports = eagerNativeRoutes
    ? componentEntries
        .map(([, importPath], index) => `import * as routeModule${index} from '${importPath}';`)
        .join('\n') + '\n\n'
    : '';
  const routeLoaders = componentEntries
    .map(([componentKey, importPath], index) => {
      const loader = eagerNativeRoutes
        ? `() => Promise.resolve(routeModule${index})`
        : `() => import('${importPath}')`;
      return `  ${JSON.stringify(componentKey)}: ${loader},`;
    })
    .join('\n');
...
function watchRoutesConfig(projectRoot = process.cwd()) {
  ...
  const regenerate = (eventName, filePath) => {
    ...
    regenerationTimer = setTimeout(() => {
      ...
      generateRoutesConfig(projectRoot);
    }, 50);
  };
